...
func (s *Store) GetRandomHighlights(n, userID int) ([]*Highlight, error) {
	...
	for rows.Next() {
		h := new(Highlight)
		if err := rows.Scan(&h.ID, &h.Text, &h.Location, &h.Note, &h.UserID, &h.BookID, &h.CreatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) GetUserByID(userID int) (*User, error) {
	row := s.db.QueryRow("SELECT id, email, firstName, lastName, created_at FROM users WHERE id = ?", userID)
	user := &User{}
